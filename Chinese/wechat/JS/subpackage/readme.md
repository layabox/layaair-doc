...
JS项目的src目录下代码在发布的时候，如果在`bin/index.html`里有引用。会与引擎库一起统一合并到code.js里。不在`bin/index.html`里引用则直接复制到js目录下。所以，root里`js/b/`在src目录下直接创建一个b目录就好了，然后创建一个game.js，这作为小游戏的分包入口文件，再创建一个实体的分包逻辑js，这里为b.js。如图3所示。

> 尽管root中可以指定具体js文件为入口，但是考虑到分包内可能会有多个js，所以还是采用了目录的默认入口game.js。

![图3](img/3.png) 

(图3)

如果开发者不小心点了编译功能，会自动把创建的js添加到`bin/index.html`里，所以最后还要检查一下，index.html里是否存在分包的代码。如果有，注释或者删除。如图4所示。

![图4](img/4.png) 

（图4）

这一步的检查还是挺重要的，如果不重视，有可能问题会出在这里。
...
下面，开发者可以先把我给的示例项目，进行分包尝试。在小游戏里跑通。真正的理解分包之后，再进行自由的实战小游戏分包。如果遇到问题请把问题发到社区，并在问题中上传示例DEMO项目，群里可以@ 管理员 charley提供链接。
