...
导出到项目后，自动打包好的图集默认位于“`项目根目录/bin/h5/res/atlas/`”目录下，图集命名与打包工具中的图集命名一样，以目录名为图集名称，如图7-1所示。
...
例如：上文中图6的资源打包后如图1所示，现在我们将图6-1中`ui`目录下的小图`monkey1.png`在项目中通过图集的方式显示出来，示例代码如下：
