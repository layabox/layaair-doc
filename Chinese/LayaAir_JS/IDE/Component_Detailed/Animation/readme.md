...
动画播放起始位置的帧索引默认是从0开始，index属性中的值可以设置为动画中的任意一帧。设置后，将从指定的帧开始播放动画。



#### 1.8 设置混合模式（blendMode）
