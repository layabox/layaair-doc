...
2、下载安装小米的测试APP (暂时还没有公开地址，可以联系QQ\微信 399050索取）
...
简单提示一下，下载后的压缩包，解压放到一个路径简单的目录。要记得添加环境变量（不知如何添加环境变量的可自行百度）。
...
当我们能看到`Debugger URL opened in Chrom.`这个提示的时候，说明chrome已被成功调起，并与真机中的画面保持一致。如图7所示。真机中的各种操作，会完全同步到PC这个chrome里，我们就按chrome的调试方式来调试小米快游戏项目即可。
