...
截止到本篇文档撰写时，当前的12.x版与Gulp环境冲突，会导致本文中的命令行编译无法执行，所以，如果开发者没下载安装nodejs的，请直接下10.x版本，且必须是10.x版本的，如果安装了其它node版本的。请卸载本地版本。重新下载10.x版本进行安装。
...
以上问题，当前通过替换compile.js库，或者未来更新至2.4.0正式版或更高版本，即可解决。
...
安装命令需要去掉全局参数`-g`，另外，本地gulp环境，不能安装4.x的版本，这里我们推荐使用3.9.1，所以最终本地gulp的安装命令为： `npm i gulp@3.9.1`，安装效果如下图所示。

![图](img/20.png) 

我们可以对比一下上图中红色圈出的部分，未安装本地gulp版本前，本地gulp版本号为Unknown，安装后版本显示为3.9.1，说明我们的gulp本地环境安装成功了。

安装完成后，可以看到项目目录内多了一个`node_modules`目录，如下图所示。
