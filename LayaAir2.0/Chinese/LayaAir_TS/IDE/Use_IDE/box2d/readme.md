...
> Author: Charley    Date: 2020-12-2   Engine&IDE Version : 2.9.0
...
static类型的刚体就是**始终静止不动**，无论施加怎样的力都不会移动， **不受重力影响**，**速度为零， 质量为零 **。 可以通过代码对节点的移动，旋转，缩放，对其进行控制。
...
kinematic类型的刚体**不会根据受到的力进行移动**，但可以被用户手动移动，**不受重力影响，可设置速度，质量为零。**通常会通过设置速度使其移动。
...
阻尼是外界作用或自身原因引起逐渐下降的衰减特性，刚体的角阻尼则是相对于角速度的旋转阻尼系数，使得旋转速度衰减。范围从0到无穷大。默认值为0，表示没有阻尼，如图2-4。通常阻尼的值在0到0.1之间。
...
mask值是所有碰撞类别category按位AND的值，这块理解起来需要用到对位运算的知识，我们也可以用碰撞类别category值求和来简单快速的处理。

假如，ABCD四个刚体，碰撞类别category分别为1、2、8、4。那刚体E要与ABC发生碰撞，则E的mask应设置为11。要与ABCD都发生碰撞，则mask应设置为15。要与ABD发生碰撞，则mask应设置为7。
...
如果我们设置了锚点`anthor`的属性值为中心点（例如50,50），无论鼠标点中刚体的什么位置，都会以锚点为约束该关节的控制点。如动图15-3所示。

![img](img/15-3.gif)  

 （动图15-3）

> `anthor`是只读属性，只是在首次设置值有效（比如在IDE中设置属性值），不支持动态修改属性值。
...
最大力是为到达线性偏移`linearOffset`位置而施加的力，默认值为1000，如果力为0，则无法到达偏移位置。

最大扭力是为到达角度偏移 `angularOffset`位置而施加的力。默认值为1000，如果扭力为0，则无法到达偏移的角度。

之前的动图19-2与动图19-4，就是分别施加了最大力与最大扭力的效果。
...
`onTriggerEnter`是在发生物理碰撞时，自动执行的生命周期虚方法。

### 3.2 onTriggerStay

`onTriggerStay`是在发生持续的物理碰撞时，自动执行的生命周期虚方法。
