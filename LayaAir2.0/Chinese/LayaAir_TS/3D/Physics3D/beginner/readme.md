...
那么将只会触发物理反馈（例如其它物体撞击后引起其它物体的反弹），但自己不会受力的影响，不会产生受力位移，位移只能通过transform改变节点。常用于游戏中的移动平台，刚体动画等应用场景。
...
动力学刚体在同等的重力下，物体的质量越大，下落速度越快。如动图1-1右侧所示：
...
动力学刚体在同等的质量下，重力越大，下落的速度越快。对比效果类似动图1-1。
...
阻尼是外界作用或自身原因引起逐渐下降的衰减特性，刚体的角阻尼则是相对于角速度的旋转阻尼系数，使得旋转速度衰减。动图1-3，是在同样的31.4角速度下，左侧为1的的角阻尼值，右侧为0.9的角阻尼值，对比效果。
...
运动刚体碰撞器从表象上看，与静态碰撞器基本上没有什么区别。都是不受重力、不受速度、不受其它力的影响，在物理世界中永远处于静止，只能通过transform去改变节点坐标来移动。但实质上，静态碰撞器的应用场景就是要永远不动，如果通过节点去移动静态碰撞器，将会比较消耗性能。如果有移动的碰撞器需求，例如来回移动的跳板或障碍，使用运动刚体碰撞器就可以了。
...
这些组件包括了碰撞形状，无需通过引擎代码添加碰撞形状，所以对于盒形、球形、胶囊形、网格形。建议在Unity里编辑导出使用。
...
复合碰撞形状是由多个基础形状组合而成的碰撞器形状。例如桌子或者凳子等，可以由多个盒形碰撞形状组成，如图12-1所示。

![img](img/12-1.png) 

（图12-1）

创建复合碰撞形状的方式并不复杂，先实例化复合碰撞形状`CompoundColliderShape()`，再通过复合碰撞形状对象的addChildShape方法添加基础碰撞形状子对象即可，后面会有代码示例说明。
...
复合碰撞形状可以添加多个不同的子形状，理解后其实也是非常简单，我们继续通过代码和注释来理解。编写代码如下所示：
...
生命周期是从开始到结束的完整周期过程，碰撞生命周期是碰撞的开始到碰撞的结束的各阶段完整历程。引擎在每个生命周期的关键触发点都提供了一个虚方法，在某个条件达到时自动激活。开发者直接重写这些虚方法，即可在条件达到时触发对应的代码逻辑。

##### 2.4.1 碰撞生命周期方法说明

前文介绍过，检测物理碰撞的方式有两种，那碰撞的生命周期，也对应着两种。分别是碰撞器生命周期方法和触发器生命周期方法。

###### 碰撞器生命周期方法说明：

| 碰撞器生命周期方法名称 | 碰撞器生命周期方法说明                                       |
| ---------------------- | ------------------------------------------------------------ |
| onCollisionEnter       | **刚发生物理碰撞时**，也就是碰撞器生命周期内的第一次进入碰撞，自动执行的生命周期虚方法，该方法只会执行一次。 |
| onCollisionStay        | **持续的物理碰撞时**，也就是碰撞器生命周期内的第二次碰撞到碰撞离开前，自动执行的生命周期虚方法。该方法在持续碰撞期间，每帧都会执行。 |
| onCollisionExit        | **物理碰撞结束时**，自动执行的生命周期虚方法，该方法只会执行一次。 |

###### 触发器生命周期方法说明：

| 碰撞触发器生命周期方法名称 | 碰撞触发器生命周期方法说明                                   |
| -------------------------- | ------------------------------------------------------------ |
| onTriggerEnter             | **刚发生物体接触时**，也就是碰撞触发器生命周期内的第一次进行接触，自动执行的生命周期虚方法，该方法只会执行一次。 |
| onTriggerStay              | **持续的物体接触时**，也就是碰撞触发器生命周期内的第二次接触到接触离开前，自动执行的生命周期虚方法。该方法在持续接触期间，每帧都会执行。 |
| onTriggerExit              | **物体接触结束时**，自动执行的生命周期虚方法，该方法只会执行一次。 |

###### 特别说明：

- 碰撞器的生命周期方法永远不会与触发器的生命周期方法同时激活，要么是碰撞器，否则是触发器，只能激活对应的生命周期方法。
- 无论是碰撞器还是触发器的生命周期方法，从进入到离开的顺序皆为“Enter,Stay,Stay,……,Exit”。

##### 2.4.2 碰撞器生命周期方法的触发条件

根据碰撞器的类型不同，并不是所有碰撞器之间，都会触发碰撞的反馈，以及激活相应的生命周期方法。

下面通过表格的方式，对应了各碰撞器之间是否可触发碰撞生命周期。
...
LayaAir引擎支持Unity中固定关节`Fixed Joint`的三个属性（Connected Body、Break Force、Break Torque）是支持导出使用的。如下图13所示。
...
LayaAir引擎支持Unity中可配置关节的属性如图14的红框中所示。下面将逐一详细介绍。
...
检测一个和所有的区别比较容易理解，就是碰到第一个物体后射线立即结束，和射线可穿透所有碰撞物体一直不结束，这两种区别。如图15所示。
...
如何设置碰撞组值collisonGroup和如何指定可发生碰撞的组值collisionMask在前文中已经介绍过，
...
Cannon.js物理引擎与Butllet的使用方式基本相同，只是API名称会有所不同，本小节列出常用的API供开发者参考。
