...
虽然Unity没有复合碰撞体，但是，开发者在Unity中，对同一个节点对象添加多个碰撞体（例如同时添加`Box Collider`组件和`Sphere Collider`组件，那通过LayaAir的导出插件导出后，会自动识别为复合碰撞形状）
...
- 碰撞事件的生命周期方法永远不会与触发事件的生命周期方法同时激活，要么是碰撞事件，否则是触发事件，只能激活对应的生命周期方法。
