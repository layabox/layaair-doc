...
有开发者问过LayaAir引擎是否会有自己的3D场景编辑器，首先可以肯定的讲，一定会有。3D编辑器是未来LayaAir3.x引擎的基础部分。但目前的LayaAir 2.x引擎仍会以引擎功能为最高优先级，为什么呢？

因为当前H5与小游戏领域的3D引擎在功能方面，与Native领域的引擎还有差距，LayaAir作为在3D领域领跑小游戏与H5产业的游戏引擎，更紧急的目标是在功能上尽可能加速丰富，让游戏产业3D小游戏产品的表现更自由、更丰富、更炫酷。

并且，当前的3D可视化编辑方案，是基于Native引擎中最成熟的3D游戏编辑工具Unity，开发者在Unity中编辑后通过LayaAir引擎插件直接导出即可使用。既方便大量拥有Unity研发经验的优质开发者快速进入H5与小游戏领域，也因为Unity工具的成熟度，充分满足当前开发者的需求。

所以，在充分满足了完整的3D游戏开发工作流的前提下。LayaAir引擎2.x版本将继续加快引擎功能的丰富，继续领跑产业，帮助更多开发者可以生产出更优质的3D产品。

但是LayaAir 3.x引擎一定会推出易用性更强，为LayaAir引擎量身制作的3D可视化开发工具。

本篇，将会为熟悉Unity和不熟悉Unity的开发者提供了，可视化3D编辑工具与插件使用的全面介绍。不仅可以满足新手开发者，对可视化工具的初步认知（不理解的也可以通过关键字百度答案）。也可以满足熟悉Unity的开发者，充分了解LayaAir引擎支持的插件，到底支持了哪些Unity功能属性的使用。

希望大家可以认真阅读完。

### 二、LayaAir的3D场景编辑环境配置

在当前，LayaAir引擎创建3D项目和编辑UI是基于自研的LayaAirIDE。编辑3D场景是基于Unity以及LayaAir引擎提供的Unity插件。所以本小节引导大家安装好3D场景的编辑与导出环境。
...
##### 2.3.1 创建一个空项目（本步骤不可跳过）

除非是在已经安装过LayaAir3D插件的Unity项目上进行升级安装，否则，

无论打算创建一个新项目，还是要导入老的Unity项目。**都要先新建一个空项目。**

想直接打开一个Unity已有项目，在进行插件安装的做法是错误的，通常会导致插件安装不成功。

如果想使用已有项目的场景资源和编辑好的场景，也要先在空项目里安装好LayaAir引擎的插件，然后再把旧项目的资源目录（也就是Unity项目中的Assets目录）复制到新建的项目里。
...
点击`打开`后，会有插件包含的内容列表展示，如果有我们不想要的功能，比如在Unity里直接预览这个功能不想要，也可以把默认的勾选去掉，但是我们建议全部导入（默认就是）。
...
`绑定设备`是插件自动识别的，无需手动绑定，但如果设备绑定达到帐号上限，需要切换设备时，需要点击`解绑`，才能在其它的设备上登录该帐号。此时当前设备则无法再登录成功。
...
LayaSupoortList目录下是之前介绍帮助菜单时，对应Tutorial（插件导出功能说明）的本地文件。
...
是不是真的成功导出，还要留意Console面板中，在导出时是否有报错或者警告，如果有，需要解决好问题后再重新导出。
...
Scene与Sprite3D分别是场景导出的配置项和预设导出的配置项。如下图所示。

![img](img/3.3-4.png) 

##### `Scene 场景 `

采用`Scene(场景)`选项导出，会导出整个场景，无论场景中的模型、材质、贴图、动画、还是光照贴图全部导出。主要用于整个场景的制作导出和直接使用，导出后的文件扩展名是`.ls`。在LayaAir引擎中，需要用Scene类或它的继承类加载。

##### `Sprite3D 预设`

Sprite3D选项，是指的Unity中的预设导出，中文语言版本这里显示的就是预设，由于该项导出后需要以LayaAir引擎的Spite3D类来加载，所以英文语言版一直采用Spite3D来表示Unity中的预设导出项。
...
如果项目中不使用顶点色和法线贴图，勾选相关选项后，可以节省20%左右的模型资源大小。VIP专属的模型压缩功能，可以降低3D模型文件尺寸约60%。
...
##### `Previously Skeletal Animation` 预烘焙骨骼动画

在使用普通的骨骼动画模式时，骨骼动画会受到设备性能的限制，比如在不支持JIT的iOS设备环境下，每24块骨骼就需要分mesh来渲染，否则会就出现发黑现象。

所以，插件中提供了`reviously Skeletal Animation（预烘焙骨骼动画）`功能，如下图所示。

![img](img/3.3-9-2.png) 

预烘焙骨骼动画模式是通过预先烘焙计算以及instance动态合批等技术来减少GPU的性能压力，对比常规的骨骼动画模式，预烘焙骨骼动画模式从CPU到GPU性能都有显著的优化效果，尤其是CPU计算方面更为明显。比如骨骼动画在iOS上，再也不需要通过分mesh来处理了。
...
默认的情况下，`Conventional`是必须勾选的。这里不允许取消，其作用是把没有透明通道需求的`.png`纹理资源自动转换成为`.jpg`纹理资源。这样可以避免浪费掉的文件尺寸。JPG质量比较容易理解，100为最高质量，数字越低压缩率越大，75是默认值，开发者可以根据项目需求自行调节。

`Android`与`iOS`是指输出对应系统平台的纹理压缩格式文件，Android系统这里采用的是ETC1压缩方式，iOS采用的是PVR压缩方式。在使用时，要判断不同系统平台来分别加载对应的纹理压缩资源文件。如果勾选了这两个选项，默认的`Conventional`选项可以不用勾选。
...
对比上面的图和表格，我们看到`Tag`是不支持的。`Static`与`Layer`是部分支持。	

这里再特别说一下，静态Static的勾选后是全选，如下图所示。但LayaAir引擎此时其实只认Everything。

![img](img/4.1-2.png) 

那开发者也许会认为，那我直接单选Everything是不是也可以。事实上是不行的，因为我们是依据Static的勾选状态判断，默认为Everything。所以尽管Everything、Lightmap Static的功能是支持的，但此处选择并没有效果，只需要把Static前的选项框勾选上即可。
...
上图中红框部分为支持导出的，对应了上面的支持列表。其它不在列表中的相机属性，LayaAir引擎插件暂时还不支持导出。
...
之前在介绍基础属性的`Static`里有讲过，只支持不勾选或勾选的状态，如果勾选，也只能是`Everything`，单选是不支持的。而这里的`Lightmap Static`，相当于起到了`Static`里的单选`Lightmap Static`效果。

> 对于新手，需要提醒的是，需要先到`Lighting`面板中点击`Generate Lighting`按钮烘焙光照贴图后，勾选Lightmap Static才能看到Baked Lightmap的相关属性。
...
| 文件的后缀 | 导出文件类型的说明                                           |
| ---------- | ------------------------------------------------------------ |
| .ls        | **场景文件**，选择导出`场景Scene`类别时生成的文件类型。会导出完整的场景信息，其中包含了场景需要的各种数据、光照贴图、模型、位置等。所以，需要导出场景相关的设置时，必须要使用Scene类别导出，就可以看到`.ls`后缀的文件。 |
