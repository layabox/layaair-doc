...
每次官网中要推出beta版，github中就会建立一个新的版本分支。同时上一个分支版本也会更新为release版并停止后续更新。比如，写本文的时候，官网最新版本是LayaAir 2.4.0 beta版，其它版本都是release版，所以上图中的release_2.2.0与release_2.3.0与官网中的release版是同步的，都不再继续更新，如果更新，也会产生新的版本分支，而不会在已经发布了release版本的旧分支（如2.2.0和2.30）上进行维护。

但是这里特别说明一下的是，当前github上最新的分支版本release_2.4.0并不是release版，而是对应官网上的LayaAir 2.4.0 beta。我们可以这样理解，github上的版本分支，只是官网中已发布的版本对应的引擎源码项目而已。真正的日常开发与更新提交全在主干分支master中进行。

> 对于想在已发布的公开版本分支上修改维护的开发者，可以去clone（克隆）使用分支版本。如果想使用最新版本或者想与官方团队一起维护最新版本，则使用默认的master主分支即可。

为了避免开发者误会github中最新的版本分支（如当前的release_2.4.0）对官网中发布的release版，后续的版本分支名称将会更改为LayaAir_2.x.x。
...
至于如何克隆和拉取LayaAir引擎，这种比较基础的Git操作，本篇就不展开介绍了，大家如有疑惑可以百度查询。

推荐使用来克隆LayaAir引擎的开源项目，关于Github帐号的创建以及Git工具的基础使用，本篇就不展开介绍了，大家如有疑惑可以百度查询。
...
引擎团队用于搭建引擎自动测试的工程，当前仍在开发调整中，**开发者无需关注。**
...
如果是绿色文字提示`Albe to merge`，说明是可以合并的状态。如果是红色文字提示`Can’t automatically merge.` 那说明有冲突，要先解决好冲突再来提交。
...
写这篇文档，自然是希望更多的人来参与到LayaAir开源生态的建设中来，如果您有源码的维护能力，希望积极参与beta版本的体验，一起发现BUG与修复BUG，然后提交到github中，让更多的人受益。引擎源码是的成熟与稳定是大家共同的财富。
