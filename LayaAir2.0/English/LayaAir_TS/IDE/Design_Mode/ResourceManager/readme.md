...
​`常用功能按钮`At the bottom of the resource manager are the commonly used function buttons, including opening directories, setting default properties, refreshing the list of resources, keyword filtering.
...
All resources in resource manager can be managed in IDE or directly in system. Therefore, when`选中`When directories or resources are available,`右键菜单`Click inside`打开所在目录`Or directly click to open the`功能按钮`Open it directly. The operation is shown in Figure 2.
