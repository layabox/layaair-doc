...
The default value is`2048×2048`, which determines the maximum size of a single atlas. If there are too many original images, which exceed the maximum width and height of a single atlas, new atlas files (multiple atlases) will be generated when packaged.
...
After wrapping the atlas, the atlas-specific resources (with the same name, respectively) are generated.`.atlas`Files (or`.json`Document and`.png`Documents) and for Atlas packers`rec`Files (* This rec file package software is used, developers do not care *), as shown in Figure 6-2 above.
