...
-**Write Array Buffer**(array buffer, offset, length)
...
-**WriteByte**(value: number) Writes a byte in the byte stream.
...
-**WriteFloat32**(value: number) Writes the Float32 value at the current byte offset position. The range is $ left [- 2 ^{128}, 2 ^{127}  right]$, about - 3.4E38 - 3.4E + 38.
...
-**WriteFloat64**(value: number) Writes float64-digit values ranging from - 1.7E308 to 1.7E+308.

-**WriteInt16**(value: number) writes the int16 value at the current byte offset location. Range - 32768 to + 32767.
...
-**WriteInt32**(value: number) Writes the Int32 value at the current byte offset position. -Signed integer between 2147483648 and + 2147483647.

-**WriteUint16**(value: number) Writes the Uint16 value at the current byte offset position.

-**WriteUint32**(value: number) Writes the Uint32 value at the current byte offset position.

-**WriteUint8**(value: number) Writes the Uint8 value at the current byte offset position.

-**Writeutfbytes**(value: string) Writes a string, which is read by the readUTFBytes method.

-**WritteUTFString**(value: string) Writes UTF-8 strings to the byte stream.
...
-**Bytesavailable * *: number [read-only] the number of bytes of data that can be read from the current location of the byte stream to the end.
...
Next, we will demonstrate the application of this class through a complete code. For example, in network connection, we receive and send network messages.
...
##-**Uint16Array ()**; type array represents a binary complement 16 bit unsigned array **Int32Array ()**Type arrays represent 32-bit signed arrays of binary complements
