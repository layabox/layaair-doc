...
Search in API`laya.display.Graphics`Class to see the API's various vector drawing methods. The "drawRect ();" method is used to draw vector rectangle. A detailed description of the method is shown in the following figure:
...
From the point of view of drawing rectangle, drawPath method is certainly more convenient than drawRect method. But you can use this example to understand the relevant parameter usage. As for other non rounded figures, you can code by yourself and experience by adjusting parameters.
