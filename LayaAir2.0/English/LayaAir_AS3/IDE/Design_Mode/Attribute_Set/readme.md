...
Usually in public attributes`var`,`name`,`eventscript`,`位置`。 As shown in Figure 2-1.
...
![图片1.png](img/5-2.png)< br / > (Figure 5-2)



##4. Rotation and Scaling Properties

Rotation and zoom attributes are often used in game UI, especially in IDE animation.
...
ScaleX and Scaley scale horizontally and vertically with the pivot point as the center.
...
For example, the layaairide software we use, many panels in the software, such as attribute setter, explorer, project manager, etc., have many node sub objects, but they are not changed very frequently, so we use cacheas to cache, which improves rendering efficiency.
...
| label Stroke | Text stroke width (in pixels). The default value of 0 means no edge stroke. A kind of
